...
| 모듈 | 역할 | 상세 |
|------|------|------|
| `lib/reactive` | 시그널·파생값·부수효과 (의존성 자동 추적) | `${CLAUDE_SKILL_DIR}/references/reactive-store.md` |
| `lib/hooks` | 생명주기에 묶인 훅 17종 (React 훅 대응) | `${CLAUDE_SKILL_DIR}/references/hooks.md` |
| `lib/store` | 서버 상태 보관 + 경로 단위 구독 | `${CLAUDE_SKILL_DIR}/references/reactive-store.md` |
| `lib/view` + `lib/router` | 화면 생명주기와 라우팅 | `${CLAUDE_SKILL_DIR}/references/view-router.md` |
| `lib/form` | 스키마 기반 폼 렌더러 | `${CLAUDE_SKILL_DIR}/references/form-http.md` |
| `lib/http` | REST + zod 응답 검증 | `${CLAUDE_SKILL_DIR}/references/form-http.md` |
| `lib/sse` | 직접 구현한 SSE 클라이언트 | `${CLAUDE_SKILL_DIR}/references/sse.md` |
