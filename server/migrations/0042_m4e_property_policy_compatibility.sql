-- M4-E1 policy v4 exact-clones the C4 finance rules. Carry the typed property-tax
...
INNER JOIN policy_set AS source_policy
    ON source_policy.policy_key = 'dev-unranked-kr-individual-property-2026-v3'
...
WHERE target_policy.policy_key = 'dev-unranked-kr-individual-insolvency-2026-v4'
...
CREATE TEMPORARY TABLE m4e_property_policy_guard (
    guard_key VARCHAR(40) CHARACTER SET ascii COLLATE ascii_bin NOT NULL PRIMARY KEY,
    accepted  TINYINT UNSIGNED NOT NULL,
    CONSTRAINT ck_m4e_property_policy_guard CHECK (accepted = 1)
);

INSERT INTO m4e_property_policy_guard (guard_key, accepted)
SELECT 'v4-property-profile-exact-clone', IF(
    EXISTS (
        SELECT 1
        ...
        AND (
            ...
            WHERE source.policy_key = 'dev-unranked-kr-individual-property-2026-v3'
        )
        AND (
            SELECT COUNT(*)
...
DROP TEMPORARY TABLE m4e_property_policy_guard;
