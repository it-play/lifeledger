...
/** Keeps an unknown run-start result tied to its original mode, payload, cursor, and UUID. */
export interface RunStartRetryPolicy {
  select(snapshot: GameSnapshot, draft: RunStartDraft): RunStartRequest;
  retain(request: RunStartRequest): void;
  clear(request: RunStartRequest): void;
}
